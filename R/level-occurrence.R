...
#'
#' @param utility A character utility with the utility expression
...
#' they can occur
get_level_occurrence <- function(utility, attrs, candidate_rows) {
  # Extract useful information ----
  n_attr_lvls <- lapply(attrs, length)

  # Check if we can achieve attribute level balance
  achieve_balance <- lapply(n_attr_lvls, function(n) {
    candidate_rows %% n
  })

  if (any((do.call(c, achieve_balance) != 0))) {
    cli_alert_warning(
  ...
  # Set the level occurrence based on attribute level balance ----
  ...
  # Extract specified occurrences ----
  specified_values <- extract_specified(utility, TRUE)
  expr <- "(?<=\\])\\(.*?\\)"
  ...
  for (i in seq_along(occurrence_values)) {
    occurrence_tmp <- extract_level_occurrence(occurrence_string[[i]])
    lvls <- eval(parse(text = paste0("list", occurrence_tmp)))

    # Check if it only occurs once and repeat equal to the number of levels
    if (length(lvls) == 1) {
      # Get the first occurrence assuming that levels occur equally in all alts
      idx <- grep(paste0("\\b.*?", occurrence_names[[i]]), names(attrs))[[1]]
      lvls <- rep(lvls, length(attrs[[idx]]))
    }
  ...
  occurrence_values <- lapply(seq_along(occurrence_values), function(i) {
    occurrence_name_tmp <- names(occurrence_values[i])

    occurrence_values_tmp <- lapply(seq_along(utility), function(j) {
      if (str_detect(utility[[j]], occurrence_name_tmp)) {
        occurrence_value_tmp <- occurrence_values[i]
        names(occurrence_value_tmp) <- paste(names(utility[j]),
                                             occurrence_name_tmp,
                                             sep = "_")
        occurrence_value_tmp
      }
    })
    do.call(c, occurrence_values_tmp)
  })

  occurrence_values <- do.call(c, occurrence_values)
