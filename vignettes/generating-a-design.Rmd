...
Often, not all combinations in a full factorial makes sense. Therefore it is important to exclude the combinations of attribute levels that cannot occur in the design. There are two ways we can do this:

1) We supply a restricted candidate set using the option `candidate_set` in `generate_design`.
2) We specify restrictions in the list of options throught `opts$restrictions`, which are then applied to the full factorial. 
...
Furthermore, let us assume that both attributes cannot simultaneously take on the value of 1. To implement this restriction, we would specify: 
