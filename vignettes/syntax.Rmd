...
It is even possible to specify parameter priors as functions of earlier parameters. The only requirement is that the earlier parameter has been specified with a prior. In the example below, the prior value of `alpha` is specified as half of `beta`.

```
beta[0.5] * x_1 + alpha[beta / 2]
```
...
We combine the parameter and attribute as in the example below to start building our utility expression. It is important to note that the parameter always comes first followed by one or more attributes (in the case of interaction terms). The functions that parses and evaluates the utility expressions assumes that parameters and attributes enter in that order. 
